...
class ComplexCNN(nn.Module):
    def __init__(self):
        ...
        # Outputs
        self.fc_throttle = nn.Linear(512, 1)
        self.fc_brake = nn.Linear(512, 1)
        self.fc_steering = nn.Linear(512, 1)
    ...
    def forward(self, image_input, speed_input):
        ...
        x = F.relu(self.fc1(x))  
        x = F.relu(self.fc2(x))
        x = F.relu(self.fc3(x))  # Output from image feature extraction
        ...
        return throttle_output, brake_output, steering_output
'''
class ComplexCNN(nn.Module):
    def __init__(self):
        super(ComplexCNN, self).__init__()
