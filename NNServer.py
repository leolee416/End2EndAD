        ...
        try:
            ...
            if bytes_received > 0:
                ...
                if len(data_package.ImageData) != total_size:
                    ...
                    # throttle, brake,steering = process_client_data(data_package, auto_drive)

                    # #发送响应 (throttle;brake;steering)
                    # response = f"{throttle};{brake};{2*steering}"
                    # print(f"[INFO] Prediction: Throttle={throttle},  Brake={brake},Steering={steering}")


                    print(f"[INFO] Vehicle state: Direction ={data_package.Direction}, Position={data_package.Position.X, data_package.Position.Y, data_package.Position.Z}")
                    response = "0.2;0;0"
                    print(f"[INFO] Prediction: Throttle=1,  Brake=0,Steering=0")
                    client_socket.sendall(response.encode('utf-8'))
            else:
...
def main():
    # 初始化 AutoDrive 模块
    model_path = "models\cnn_fullinput_right.pth"
